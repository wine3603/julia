...
JL_DLLEXPORT void jl_arrayset(jl_array_t *a, jl_value_t *rhs, size_t i)
{
    ...
    if (!a->flags.ptrarray) {
        if (jl_is_uniontype(eltype)) {
            uint8_t *psel = &(jl_array_typetagdata(a) + i);
            unsigned nth = 0;
                                        ...
                                        size_t n)
{
    ...
    if (a->offset >= inc) {
        ...
        size_t oldttoffset = (a->maxsize - a->offset) * elsz + a->offset;
        size_t oldoffsnb = a->offset * elsz;
        size_t nb1 = idx * elsz;
        if (inc > (a->maxsize - n) / 2 - (a->maxsize - n) / 20) {
            ...
            if (!array_resize_buffer(a, newlen)) {
                ...
                if (isbitsunion) typetagdata = data + oldttoffset;
            }
            newdata = (char*)a->data + newoffset * elsz;
            if (isbitsunion) newtypetagdata = newdata + a->maxsize * elsz + newoffset;
                                        ...
                                        size_t inc, size_t n)
{
    ...
    if (__unlikely(reqmaxsize > a->maxsize)) {
        ...
        if (newbuf) {
            ...
            if (isbitsunion) {
                // jl_safe_printf("oldnbytes=%i, nbytes=%i, oldlen=%i, newlen=%i\n", oldnbytes, nbytes, oldlen, newlen);
                memmove(newtypetagdata, typetagdata, idx);
                if (has_gap) memmove(newtypetagdata + idx + inc, newtypetagdata + idx, n - idx);
                memset(newtypetagdata + idx, 0, inc);
            }
